...
export function AIFloatingButton({ onClick, open }: { onClick: () => void; open: boolean }) {
  ...
  return (
    <button
      ...
      className="ai-pulse fixed bottom-20 right-4 z-[70] flex h-14 w-14 items-center justify-center rounded-full bg-gradient-to-br from-blue-500 via-blue-600 to-cyan-600 text-white shadow-[0_16px_40px_-8px_rgba(8,145,178,0.65)] transition-transform hover:scale-105 active:scale-95 sm:bottom-6 sm:right-6 cursor-pointer"
    >
    ...
    <div className="fixed inset-0 z-[85]" role="dialog" aria-modal="true" aria-label="Pulse AI assistant">
      ...
      <div className="slide-in-right glass-strong absolute inset-y-0 right-0 flex w-full max-w-md flex-col sm:rounded-l-2xl">
        ...
        <div className="flex items-center gap-3 border-b border-line px-5 py-4">
          <span className="ai-breathe flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-blue-500 to-cyan-600 text-white">
        ...
        <div ref={scrollRef} className="flex-1 space-y-4 overflow-y-auto px-5 py-4">
          ...
          {messages.map((m) =>
            m.role === "user" ? (
              <div key={m.id} className="flex justify-end">
                <div className="max-w-[85%] rounded-2xl rounded-br-md bg-gradient-to-r from-blue-600 to-cyan-600 px-4 py-2.5 text-sm text-white">
                  {m.text}
                </div>
              </div>
            ) : (
              <div key={m.id} className="rise-in flex gap-2.5">
                <span className="mt-1 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-blue-500 to-cyan-600 text-white">
          ...
          {thinking && (
            <div className="flex items-center gap-2.5 text-fog">
              <span className="ai-breathe flex h-7 w-7 items-center justify-center rounded-full bg-gradient-to-br from-blue-500 to-cyan-600 text-white">
                <Icon name="sparkles" size={13} />
              </span>
